...
Now, lets discuss each functions and see how they should be implemented. 
 ...
 - **zeros**
implement this function so that it will create a $n \times m$  matrix with all elements equal to zero.
	```cpp
	Matrix zeros(size_t n, size_t m)
	```

 - **ones**
implement this function so that it will create a $n \times m$  matrix with all elements equal to one.
	```cpp
	Matrix ones(size_t n, size_t m)
	```

 - **random**
implement this function so that it will create a $n \times m$  matrix with all elements a random number between `min` and `max`.
	```cpp
	Matrix random(size_t n, size_t m, double min, double max)
	```
	**note.** to generate a random number you are not allowed to use `rand` and `srand` functions. instead use the `<random>` library introduced in c++11.
...
- **multiply** - *matrix*
implement this function so that it multiplies the `matrix1` into `matrix2`. (this is **not** an elemnt-wise multiplication)
	```cpp
...
- **minor**
implement this function so that it will create the minor of the input `matrix` with respect to $n^{th}$ row and $m^{th}$ column.
	```cpp
	Matrix minor(const Matrix& matrix, size_t n, size_t m)
	```
	**note.** the indices in c++ start from 0 ~~not 1~~, forexample:
$$
  M = \begin{bmatrix}
 a & b & c \\
 d & e & f \\
 g & h & i
  \end{bmatrix} \implies M_{1, 1} = \begin{bmatrix}
 a & c \\
 g & i
  \end{bmatrix}
$$
...
implement this function so that it calculates the determinant of the input `matrix`. 
	...
	```
	*you should only use the minor approach to calculae the determinant, any other methods (such as upper-triangular or ...) is not allowed.*
...
$$
axis=0 \implies \begin{bmatrix}
 matrix1 \\
 matrix2
  \end{bmatrix} \:\: , \:\:
axis=1 \implies \begin{bmatrix}
 matrix1 & matrix2
  \end{bmatrix}
$$
...
we  want to implement elementary row operation (ERO) functionality for our library. as you know we have 3 EROs: `1)`swaping two rows. `2)`multiplying a row into a constant number. `3)`multiplying a row into a constant number and add it to another row.
So, lets define the following 3 functions to do this.
    - **swap**
    swaps $r_1^{th}$ row with $r_2^{th}$.
        ```cpp
        Matrix ero_swap(const Matrix& matrix, size_t r1, size_t r2)
        ```
		
    - **multiply**
    multiplies every elemnt in $r^{th}$ row with constant number `c`.
        ```cpp
        Matrix ero_multiply(const Matrix& matrix, size_t r, double c)
        ```

    - **sum**
    adds $r_1^{th} \times c$ into $r_2^{th}$ row.
        ```cpp
        Matrix ero_sum(const Matrix& matrix, size_t r1, double c, size_t r2)
        ```

- **upper triangular**
implement this function so that it will calculates the upper triangular form of the `matrix` using the ERO operations.
    ```cpp
    ...
    ***bonus question** what happens if we come across a zero on our main diagonal forexample $a$ or $j$ in the example above? solve this problem. 
    The last unit test (`BONUS`) is for this bonus dont worry too much if it fails.*
	

# Cautions
- Whenever you are defining a function you should always concider every aspect of its usage. in this homework you are writing a lbrary so it can be used by others. so you should keep in mind they may not always use your functions properly. forexample they may try to calculate the invert of a non-singular matrix, or they may try to multiply two matrices with incorrect dimensions.
You should keep track of these misbehaviours that may happen in your functions and throw a `logic error` in these cases.
if you are not familiar of how to throw errors just try to google it. dont worry it will be the simplest part of the homework :stuck_out_tongue_winking_eye:
- other than logic errors you should consider that the users may try to use empty matrices with your functions. (forexample the determinant of an empty matrix is 1). keep this in mind in all your implementations.
